...
const frames = [
  "/home/ubuntu/webdev-static-assets/video-frame-01.jpg",
  "/home/ubuntu/webdev-static-assets/video-frame-02.jpg",
];
...
for (const [index, path] of frames.entries()) {
  ...
  const matched = matchVideoTracks(tracks, detections, nextTrackId, index * 0.5);
  tracks = matched.tracked;
  nextTrackId = matched.nextTrackId;
  console.log(JSON.stringify({ frame: index + 1, detections: tracks.map(({ trackId, label, confidence }) => ({ trackId, label, confidence })) }, null, 2));
}
