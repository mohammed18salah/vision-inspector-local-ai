...
if (process.env.ELECTRON_EXPORT_HISTORY_FORMAT === "csv" || process.env.ELECTRON_EXPORT_HISTORY_FORMAT === "pdf") {
  const exportResponse = await client.send("Runtime.evaluate", { expression: `(async () => window.visionDesktop.exportHistory(${JSON.stringify(process.env.ELECTRON_EXPORT_HISTORY_FORMAT)}, await window.visionDesktop.listHistory()))()`, awaitPromise: true, returnByValue: true });
  historyExport = exportResponse.result.value;
}
...
response.result.value.historyExport = historyExport;
