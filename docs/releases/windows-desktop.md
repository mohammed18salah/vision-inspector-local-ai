...
Download the `Vision Inspector Local AI Setup *.exe` installer and verify its SHA-256 checksum against `SHA256SUMS.txt` before installation. This first public desktop build is not Authenticode-signed, so Windows SmartScreen may show a warning. Do not bypass a warning unless you downloaded the EXE from this repository's official GitHub Release and verified the checksum.
