...
export function DesktopApp() {
  ...
  const exportHistory = useCallback(async (format: "csv" | "pdf") => {
    if (!history.length) return;
    try {
      const outcome = await window.visionDesktop.exportHistory(format, history);
      if (outcome.saved) { setHistoryError(""); setMessage(`حُفظ سجل التاريخ ${format.toUpperCase()} محليًا · SHA-256 ${outcome.checksum?.slice(0, 10)}…`); }
    } catch { setHistoryError(`تعذر تصدير السجل بصيغة ${format.toUpperCase()}.`); }
  }, [history]);
