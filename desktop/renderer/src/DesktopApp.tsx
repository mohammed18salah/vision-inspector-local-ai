...
import { Activity, BrainCircuit, Check, ChevronLeft, ChevronRight, Cpu, Download, FileImage, FileText, FolderOpen, Gauge, Image, Loader2, MonitorCog, Pause, Play, ScanText, ShieldCheck, Sparkles, Video, Volume2, X, Zap } from "lucide-react";
import { type Box, type Detection, type DeviceRuntime, type OcrResult, type TrackedDetection, detectObjects, detectOpenCandidates, getPreferredDevice, matchTracks, mergeDetections, recognizeText, selectedInferenceDevice, toCsv } from "./vision";
...
export function DesktopApp() {
  ...
  const pickFile = useCallback((nextMode: Mode) => {
    setMode(nextMode); setStatus("idle"); setMessage("يفتح مربع اختيار الملف المحلي…"); setDetections([]); setOcr(null); setTracks([]); tracksRef.current = []; nextTrackRef.current = 1; setSelected(null); setVideoError(""); setTracking(false);
  ...
  const onFileChosen = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setTracks([]); tracksRef.current = []; nextTrackRef.current = 1;
  ...
  const exportResult = async (format: "json" | "csv") => {
    ...
    if (outcome.saved) setMessage(`حُفظ ${format.toUpperCase()} محليًا · SHA-256 ${outcome.checksum?.slice(0, 10)}…`);
  };

  const changeMode = (nextMode: Mode) => { setMode(nextMode); setFile(null); setDetections([]); setTracks([]); tracksRef.current = []; nextTrackRef.current = 1; setOcr(null); setStatus("idle"); setMessage("اختر ملفًا محليًا لبدء التحليل."); setTracking(false); };
  ...
  return <div className="desktop-shell"><input ref={fileInputRef} type="file" hidden accept={mode === "image" ? "image/jpeg,image/png,image/webp" : "video/mp4,video/webm,video/quicktime"} onChange={onFileChosen}/>
    <header className="desktop-titlebar"><div className="desktop-brand"><BrainCircuit size={19}/><strong>Vision Inspector</strong><span>LOCAL AI · WINDOWS</span></div><div className="desktop-device"><span className={`device-dot ${runtimeDevice === "webgpu" ? "device-dot-ready" : ""}`}/><span>{deviceState}</span><b>{deviceName}</b></div></header>
    <div className="desktop-workspace">
      <aside className="desktop-nav"><div className="nav-caption">WORKSPACE</div><button className={mode === "image" ? "nav-active" : ""} onClick={() => changeMode("image")}><Image size={18}/>تحليل صورة</button><button className={mode === "video" ? "nav-active" : ""} onClick={() => changeMode("video")}><Video size={18}/>تحليل فيديو</button><div className="nav-divider"/><div className="nav-caption">ENGINE</div><div className="engine-card"><Cpu size={17}/><span>المحرك الفعلي</span><strong dir="ltr">{deviceName}</strong><small>{runtimeDevice === "webgpu" ? "WebGPU active" : "CPU / WASM fallback"}</small></div><div className="privacy-card"><ShieldCheck size={17}/><p><strong>خصوصية محلية</strong>الوسيط يبقى على جهازك. التنزيل الأول للنموذج فقط يحتاج اتصالًا.</p></div></aside>
      <main className="desktop-main"><section className="desktop-command"><div><span className="eyebrow">{mode === "image" ? "IMAGE INSPECTOR" : "VIDEO TRACKING"}</span><h1>{mode === "image" ? "حلّل صورة بدقة محلية." : "تتبّع الكائنات عبر الفيديو."}</h1><p>{mode === "image" ? "كشف الكائنات، OCR، تصدير، ومربعات مرتبطة بمحتوى الصورة الحقيقي." : "إطارات محلية، مسارات ثابتة، ومشغل يحافظ على صوت الفيديو الأصلي."}</p></div><div className="command-actions"><button className="fs-button fs-button-secondary" onClick={() => void pickFile(mode)}><FolderOpen size={16}/>اختيار {mode === "image" ? "صورة" : "فيديو"}</button>{mode === "image" ? <button className="fs-button fs-button-primary" disabled={!file || status === "loading" || status === "analyzing"} onClick={() => void analyzeImage()}><Zap size={16}/>{status === "loading" || status === "analyzing" ? "جارٍ التحليل…" : "ابدأ التحليل"}</button> : <button className="fs-button fs-button-primary" disabled={!file || tracking} onClick={() => void startTracking()}><Play size={16}/>ابدأ التتبع</button>}</div></section>
